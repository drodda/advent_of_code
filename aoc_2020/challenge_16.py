...
def main():
    ...
    log.debug("")
    log.debug("Ordered rules:")
    log.debug(col_rules)

    part2_result = 1
    log.debug("")
    log.debug("Own ticket:")
    for i, rule in enumerate(col_rules):
        if rule.name.startswith("departure "):
            log.debug(f"{i}: {rule}, {own_ticket[i]}")
